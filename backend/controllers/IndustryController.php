...
use backend\models\IndustrySearch;
...
/**
 * SiteTypeController implements the CRUD actions for SiteType model.
 */
class IndustryController extends Controller
{
    ...
    {
        $model = $this->findModel($id);
        $model->status = Industry::STATUS_DISABLE;
        $model->save();
        return $this->redirect(['index']);
    }
    ...
    {
        $model = $this->findModel($id);
        $model->status = Industry::STATUS_ACTIVE;
        $model->save();
        return $this->redirect(['index']);
    }
            ...
            if($file !== null)
            {
                if($validator->validate($file))
                {
                    ...
                    foreach($data as $index => $row)
                    {
                        ...
                        //查询是否存在
                        $industry=Industry::findOne(['name'=>$attrs['name']]);
                        if($industry != null) {
