...
use Yii;
use backend\models\Paragraph;
use backend\models\ParagraphSearch;
...
class ParagraphController extends Controller
{
    ...
    {
        $model = $this->findModel($id);
        $model->status = Paragraph::STATUS_ACTIVE;
        $model->save();
        return $this->redirect(['index']);
    }
                ...
                if($validator->validate($file))
                {
                    ...
                    foreach($data as $index => $row)
                    {
                        ...
                        if($attrs['typename']=="文章"){
                            $attrs['type']=Paragraph::TYPE_ARTICLE;
                        }elseif($attrs['typename']=="站内站"){
                            $attrs['type']=Paragraph::TYPE_STATION;
                        }else{
                        ...
                        // 查询段落是否存在
                        $paragraph=Paragraph::findOne(['content'=>$attrs['content'],'product_id'=>$attrs['product_id']]);
                        if($paragraph != null) {
